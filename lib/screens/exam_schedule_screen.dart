...
class _ExamScheduleScreenState extends State<ExamScheduleScreen> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Exam Schedule', style: TextStyle(fontWeight: FontWeight.bold)),
          ...
          return Column(
            children: [
              ...
              if (provider.selectedSemesterId != null &&
                  provider.examScheduleData?.semesterId != provider.selectedSemesterId)
                Padding(
  ...
  Widget _buildContent(VtopDataProvider provider) {
    ...
    if (provider.error != null) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          Text(provider.error!, style: TextStyle(color: isDark ? Colors.white70 : Colors.black54), textAlign: TextAlign.center),
          const SizedBox(height: 16),
          FilledButton.icon(onPressed: () => provider.fetchExamSchedule(),
    ...
    if (exams.isEmpty) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.event_note, color: isDark ? Colors.white30 : Colors.black26, size: 64),
          const SizedBox(height: 16),
          Text('No exam schedule data', style: TextStyle(color: isDark ? Colors.white54 : Colors.black45)),
          const SizedBox(height: 12),
          FilledButton.icon(onPressed: () => provider.fetchExamSchedule(),
              icon: const Icon(Icons.download), label: const Text('Fetch Schedule')),
        ]),
      );
    }
      ...
      children: [
        Padding(
          ...
          child: Text(
            ...
            style: const TextStyle(color: AppTheme.primaryColor, fontWeight: FontWeight.bold, fontSize: 16, letterSpacing: 1),
          ),
        ),
      ...
      child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            Text(exam.examSession, style: const TextStyle(color: AppTheme.secondaryColor, fontWeight: FontWeight.w600, fontSize: 12)),
          ]),
          const SizedBox(height: 12),
          ...
          Row(
            children: [
              Text(exam.courseCode, style: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontSize: 12)),
              Text('  ·  ', style: TextStyle(color: isDark ? Colors.white38 : Colors.black38)),
              Text(exam.courseType, style: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontSize: 12)),
              Text('  ·  ', style: TextStyle(color: isDark ? Colors.white38 : Colors.black38)),
              Text('Slot: ${exam.slot}', style: TextStyle(color: isDark ? Colors.white54 : Colors.black54, fontSize: 12)),
            ],
          ),
          const SizedBox(height: 12),
          ...
          Row(children: [
            Expanded(child: _InfoCol(icon: Icons.access_time, label: 'Reporting', value: exam.reportingTime)),
            Expanded(child: _InfoCol(icon: Icons.timer, label: 'Exam Time', value: exam.examTime)),
          ]),
          const SizedBox(height: 12),
          Row(children: [
            Expanded(child: _InfoCol(icon: Icons.location_on, label: 'Venue', value: exam.venue)),
            Expanded(child: _InfoCol(icon: Icons.event_seat, label: 'Seat', value: '${exam.seatLocation} - ${exam.seatNo}')),
          ]),
        ]),
      ),
    );
...
class _InfoCol extends StatelessWidget {
  ...
  const _InfoCol({required this.icon, required this.label, required this.value});
